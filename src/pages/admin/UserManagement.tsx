    ...
    <div className="p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-3 animate-fade-up" style={{ animationDelay: "80ms" }}>
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full bg-zinc-800 border border-zinc-700 rounded-lg pl-9 pr-3.5 py-2 text-sm text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
          />
        </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden animate-fade-up" style={{ animationDelay: "120ms" }}>
          ...
          <thead>
            <tr className="border-b border-zinc-800">
              {["Member", "Role", "Actions"].map((h) => (
                <th key={h} className="px-4 py-3 text-left text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                  {h}
                </th>
              ))}
            </tr>
                ...
                <tr key={u.id} className="hover:bg-zinc-800/20 transition-colors animate-fade-up" style={{ animationDelay: `${i * 25}ms` }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="text-sm font-medium text-zinc-50">{u.name}</div>
        ...
        {!loading && filtered.length === 0 && (
          <div className="py-12 text-center text-sm text-zinc-500">No users match the current filters.</div>
        )}
      </div>
    </div>
  );
